...
vec3 addSign(vec3 color, vec3 pos, float side, vec2 id) {
    ...
    float cha = -3.5 + 8.0 * noise(id + 6.0 * charPos);
    charPos = fract(charPos);
    cha *= smoothstep(0.0, 0.4, charPos.x) * smoothstep(1.0, 0.6, charPos.x);
    cha *= smoothstep(0.0, 0.4, charPos.y) * smoothstep(1.0, 0.6, charPos.y);
    color = mix(color, signColor, flash * flicker * step(outline, 0.01) * clamp(cha, 0.0, 1.0));
...
void main() {
    ...
    vec2 xy = 2.0 * v_texCoords - iResolution.xy;
